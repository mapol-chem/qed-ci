...
class PFHamiltonianGenerator:
    ...
    def __init__(self, molecule_string, psi4_options_dict, cavity_options):
        ...
        # if doing direct method, call Davidson routine
            ...
            if self.ci_level == "cas" or self.ci_level == "fci":
                ...
                #####gradient1 = self.energy_grad(step10)
                #####gradient2 = self.numerical_orbital_gradient(step10)
                #####A_num = self.energy_grad_finite_difference_element(step10)
                #####reduced_gradient = np.zeros(self.index_map_size)
                #####index_count1 = 0
                #####for k in range(self.n_occupied):
                #####    for r in range(k+1,self.nmo):
                #####        if (k < self.n_in_a and r < self.n_in_a): continue
                #####        if (self.n_in_a <= k < self.n_occupied and self.n_in_a <= r < self.n_occupied): continue
                #####        reduced_gradient[index_count1] = A_num[r][k]
                #####        #print(r,k,index_count1)
                #####        #index_count2 = 0
                #####        #for l in range(self.n_occupied):
                #####        #    for s in range(l+1,self.nmo):
                ...
                #####for p in range(self.nmo):
                #####    for q in range(self.nmo):
                #####        print('{:.12f}'.format(self.A1111[p][q]),'{:.12f}'.format(self.A2222[p][q]),'{:.12f}'.format(A_num[p][q]))
                #####print("rqq", np.allclose(self.U_ttt,self.U_temp2, rtol=1e-15,atol=1e-15))
                #####print("test gradient")
                #####print(gradient1)
                #####print(gradient2)
                ...
                #####eps = 1e-8
                #####eps_array = np.zeros(self.index_map_size)
                #####eps_array.fill(eps)
                #####gradient3 = scipy.optimize.approx_fprime(step10, self.energy_function,eps_array)
                #####print(gradient3/2)
                #####for p in range(self.index_map_size):
                #####    print('{:.12f}'.format(gradient1[p]),'{:.12f}'.format(gradient2[p]),'{:.12f}'.format(reduced_gradient[p]),'{:.12f}'.format(gradient3[p]/2))
                # print(index_map_pq)
                # print(index_map_kl)
    ...
    def internal_optimization(self, E0, eigenvecs):
        ...
        while True:
            ...
            gradient_tilde1 = np.zeros((rot_dim, self.n_occupied))
            hessian_tilde1 = np.zeros(
                (self.n_occupied, self.n_occupied, rot_dim, rot_dim)
            )
            gradient_tilde_ai = np.zeros((self.n_act_orb, self.n_in_a))
            ...
            if gradient_norm < 1e-4 or microiteration == 40:
                ...
                self.full_transformation_internal_optimization(
                    self.U1, self.H_spatial2, self.d_cmo, self.J, self.K
                )
            ...
            mu2 = np.zeros(dim0)
            step_norm2, scale = self.internal_step2(
                gradient_tilde_ai, hessian_tilde_ai, step2, mu2, 1, dim0
            )
            print("scale", scale, flush=True)
            if np.abs(scale) > 0.05:
                # find (alpha_min, alpha_max)
                while True:
                    step_norm = self.internal_step(
                        gradient_tilde_ai, hessian_tilde_ai, step, alpha, dim0
                    )
                    print("step", step)
                ...
                if alpha_max != 1:
                    while True:
                        ...
                        if (
                            trust_radius - step_norm <= 1e-2
                            and trust_radius - step_norm >= 0.0
                        ):
                            break
                ...
                print(
                    "conditional number from product",
                    np.linalg.norm(H_lambda) * np.linalg.norm(H_lambda_inverse),
                )
    ...
    def microiteration_optimization(self, E0, eigenvecs, c_get_roots):
        ...
        while microiteration < N_microiterations:
            ...
            if (
                np.abs(current_energy - old_energy < 0.01 * convergence_threshold)
            ...
            while orbital_optimization_step < N_orbital_optimization_steps:
                ...
                step_norm2, scale = self.microiteration_step2(
                    reduced_gradient, reduced_hessian, step2, alpha, dim0
                )
                ...
                # find (alpha_min, alpha_max)
                while True:
                    step_norm = self.microiteration_step(
                        reduced_gradient, reduced_hessian, step, alpha, dim0
                    )
                ...
                if energy_change < 0.0:
                    predicted_energy1 = self.microiteration_predicted_energy(
                        reduced_gradient, reduced_hessian, step
                    )
                    predicted_energy2 = self.microiteration_predicted_energy2(
                        self.U2, reduced_gradient, A_tilde, G, step
                    )
                    print(
                        "microinteration predicted energy",
                        predicted_energy1,
                        predicted_energy2,
                        flush=True,
                    )
                    ...
                    print(
                        "sum_energy    active_one    active_two E_core active_pe_energy ci_dependent_energy E_nuc"
                    )
                    print(
                        "gfhgy",
                        "{:20.12f}".format(sum_energy),
    ...
    def microiteration_optimization2(self, eigenvecs, c_get_roots):
        ...
        while microiteration < N_microiterations:
            ...
            while orbital_optimization_step < N_orbital_optimization_steps:
                ...
                hessian_diagonal3 = np.diagonal(hessian_tilde3).reshape(
                    (self.nmo, self.n_occupied)
                )
                ...
                self.mu_min = 0.0
                trial_vector, hard_case = self.Davidson_augmented_hessian_solve(
                    self.U2,
                    A_tilde2,
                    G,
                ...
                H_op = LinearOperator(
                    ...
                    matvec=lambda Q: self.mv(self.U2, A_tilde2, G, Q, 1, 0, mu1[0]),
                )
                print(H_op)
                S = H_op.matvec(Q.T)
                print(S)
                ...
                print(S2)
                print(
                    "diagonal element of the reduced hessian",
                    np.diagonal(reduced_hessian),
                )
                ...
                energy_change1 = (
                    zero_energy + second_order_energy_change1 - current_energy
                )
                print("energy change1", energy_change1, flush=True)

                if np.abs(product) < 1e-5:
                    self.Davidson_linear_matrix_equation_solve(
                        self.U2,
                        A_tilde2,
                        G,
                        self.reduced_hessian_diagonal,
                ...
                if energy_change < 0.0:
                    ...
                    predicted_energy2 = self.microiteration_predicted_energy2(
                        self.U2, reduced_gradient, A_tilde2, G, step
                    )
                    print(
                        "microinteration predicted energy",
                        predicted_energy1,
    ...
    def microiteration_optimization3(
        ...
        while microiteration < N_microiterations:
            ...
            while orbital_optimization_step < N_orbital_optimization_steps:
                ...
                if hard_case == 0:
                    ...
                    H_op = LinearOperator(
                        (self.index_map_size, self.index_map_size),
                        matvec=lambda Q: self.mv(
                            self.U2, A_tilde2, G, Q, 1, 0, self.mu_min
                        ),
                    )
                    # S=H_op.matvec(Q.T)
                    # print(S)
    ...
    def microiteration_optimization4(
        self, U, eigenvecs, c_get_roots, convergence_threshold
    ):
        # print("E_core", self.E_core)
        self.U2 = copy.deepcopy(U)
        ...
        while microiteration < N_microiterations:
            ...
            while orbital_optimization_step < N_orbital_optimization_steps:
                ...
                start = timer()
                trial_vector, hard_case = self.Davidson_augmented_hessian_solve2(
                    self.U2,
                    A_tilde2,
                    G,
                    G1,
                ...
                if hard_case == 0:
                    ...
                    H1_op = LinearOperator(
                        ...
                        matvec=lambda Q: self.mv2(
                            self.U2, A_tilde2, G1, Q, 1, 0, self.mu_min
                        ),
                    )
                    # S=H_op.matvec(Q.T)
                    # print(S)
    ...
    def Davidson_augmented_hessian_solve(
        self,
        U,
        A_tilde,
        G,
        ...
        if restart == False:
            ...
            # print(guess_hessian)
            while True:
                mu0 = self.projection_step(
                    guess_gradient, guess_hessian, aug_eigvecs, alpha, dim1 + 1
                )
                if np.absolute(aug_eigvecs[0][0]) < 1e-4 and alpha == 1:
                    ...
                    projected_step_norm = np.sqrt(
                        np.dot(projected_step, projected_step.T)
                    )
            ...
            if alpha_max != 1:
                ...
                while True:
                    ...
                    # if count >= 100: break
                    if (
                        trust_radius - projected_step_norm <= 1e-2
                        and trust_radius - projected_step_norm >= 0.0
                    ):
                        break
            ...
            for j in range(nroots):
                # Compute a residual vector "w" for each root we seek
                w[j, :] = np.dot(eigvecs[:, j].T, S) - theta[j] * np.dot(
                    eigvecs[:, j].T, Q
                )
                residual_norm[j] = np.sqrt(np.dot(w[j, :], w[j, :].T))
                if residual_norm[j] < threshold:
            ...
            for davidson_iteration in range(1, num_iter):
                ...
                if alpha_max != 1:
                    while True:
                        ...
                        S = copy.deepcopy(S3)
                        S[:, 1:] += alpha * np.einsum(
                            "p,q->qp", reduced_gradient, Q[:, 0]
                        )
                        S[:, 0] = alpha * np.einsum(
                            "p,rp->r", reduced_gradient, Q[:, 1:]
                        )
                ...
                for j in range(nroots):
                    # Compute a residual vector "w" for each root we seek
                    w[j, :] = np.dot(aug_eigvecs[:, j].T, S) - theta[j] * np.dot(
                        aug_eigvecs[:, j].T, Q
                    )
                    residual_norm[j] = np.sqrt(np.dot(w[j, :], w[j, :].T))
                    if residual_norm[j] < threshold:
    ...
    def Davidson_augmented_hessian_solve2(
        self,
        U,
        A_tilde,
        G,
        ...
        if restart == False:
            ...
            projected_step = np.zeros(dim1)
            while True:
                theta = self.projection_step(
                    guess_gradient, guess_hessian, aug_eigvecs, alpha, dim1 + 1
                )
                normalized_eigvecs = aug_eigvecs[:, 0] / np.linalg.norm(
                    aug_eigvecs[:, 0]
                )
                bb = normalized_eigvecs[0, 0]
                aa = np.linalg.norm(guess_gradient) * np.abs(bb)
                cc = np.sqrt(1 - bb * bb)
                if aa <= 1e-4 * cc and alpha == 1:
                    # if np.absolute(aug_eigvecs[0][0]) < 1e-4 and alpha == 1:
                    hard_case = 1
                    break
                else:
        ...
        if (len(unconverged_idx) > 0 and restart == False) or restart == True:
            ...
            for davidson_iteration in range(1, num_iter):
                ...
                while True:
                    ...
                    normalized_eigvecs = full_eigvecs[0, :] / np.linalg.norm(
                        full_eigvecs[0, :]
                    )
                    bb = normalized_eigvecs[0, 0]
                    aa = np.linalg.norm(reduced_gradient) * np.abs(bb)
                    cc = np.sqrt(1 - bb * bb)
                    if aa <= 1e-4 * cc and alpha == 1:
                        # if np.absolute(full_eigvecs[0][0]) < 1e-4 and alpha == 1:
                        hard_case = 1
                ...
                if alpha_max != 1:
                    while True:
                        ...
                        # print(aug_eigvecs.T[0,:])
                        full_eigvecs = np.dot(
                            aug_eigvecs.T[:nroots, indim:], Q[indim:,]
                        )
                        full_eigvecs2 = copy.deepcopy(full_eigvecs)
                        # print("q component")
                        # print(full_eigvecs[0,:])
                ...
                w[:, 1:] += alpha * np.einsum(
                    "p,q->qp", reduced_gradient, full_eigvecs[:, 0]
                )
    ...
    def Davidson_augmented_hessian_solve3(
        self,
        U,
        A_tilde,
        G,
        ...
        H_diag = copy.deepcopy(reduced_hessian_diagonal)
        # print(H_diag)
        H_diag = np.concatenate(([alpha], H_diag))
        # print(H_diag)
        indim = 1
        maxdim = 10
        unconverged_idx = []
    ...
    def Davidson_linear_matrix_equation_solve(
        ...
        Q[0, :] = np.divide(
            -reduced_gradient, H_diag, out=np.zeros_like(H_diag), where=H_diag != 0
        )
        print(Q)
        ...
        for davidson_iteration in range(0, num_iter):
            ...
            print(
                "conditioner number of projected matrix",
                np.linalg.cond(projected_hessian),
            )
            ...
            for j in range(nroots):
                print(j + 1, residual_norm[j], convergence_check[j], flush=True)
